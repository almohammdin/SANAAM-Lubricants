...
const APP_NAME='sanaam-sales-voice';
...
window.SanaamVoiceGetAppCheckToken=async({forceRefresh=false}={})=>{const result=await getToken(appCheck,forceRefresh);if(!result?.token)throw new Error('voice-app-check-token-missing');return result.token};
window.SANAAM_VOICE_TOKEN_ENDPOINT='https://mesraah-live-token.naif123456.workers.dev/token';
